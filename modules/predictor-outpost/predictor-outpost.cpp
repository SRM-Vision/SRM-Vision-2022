...
SendPacket OutpostPredictor::Run(const Battlefield &battlefield) {
    ...
    if (is_clockwise == 0 || num < kCollectNum) {
        ...
        // Detect 1 armor.
        if (target_.size() == 1) {
            coordinate::TranslationVector moving_center_ = coordinate::TranslationVector(
                    target_[0].TranslationVectorWorld().x(), target_[0].TranslationVectorWorld().y(),
                    target_[0].TranslationVectorWorld().z());
            target_moving_center_ = CalculateMovingTargetCenter(moving_center_, 1);
            predict_shoot_center_ = CalculatePredictShootCenter(target_moving_center_, battlefield.BulletSpeed(),
                                                                battlefield.YawPitchRoll(), target_[0].Distance());
            ...
            predict_shoot_center_ = CalculatePredictShootCenter(target_moving_center_, battlefield.BulletSpeed(),
                                                                battlefield.YawPitchRoll(), going_armor_.Distance());
...
OutpostPredictor::CalculateMovingTargetCenter(const coordinate::TranslationVector &moving_center,
    ...
    if (detect_num == 1) {
        ...
        target_moving_center.x() = moving_center.x() * std::cos(is_clockwise * kArmor_included_rad_) -
                                   moving_center.y() * std::sin(is_clockwise * kArmor_included_rad_);
        target_moving_center.y() = moving_center.y() * std::cos(is_clockwise * kArmor_included_rad_) +
                                   moving_center.x() * std::sin(is_clockwise * kArmor_included_rad_);
...
coordinate::TranslationVector
OutpostPredictor::CalculatePredictShootCenter(const coordinate::TranslationVector &moving_center,
                                              float bullet_speed,
    ...
    predict_shoot_center.x() = moving_center.x() * std::cos((-1.0) * is_clockwise * rotate_angle) -
                               moving_center.y() * std::sin((-1.0) * is_clockwise * rotate_angle);
    predict_shoot_center.y() = moving_center.y() * std::cos((-1.0) * is_clockwise * rotate_angle) +
                               moving_center.x() * std::sin((-1.0) * is_clockwise * rotate_angle);
