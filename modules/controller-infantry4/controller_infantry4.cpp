 ...
 */
[[maybe_unused]] ControllerRegistry<Infantry4Controller>
        Infantry4Controller::infantry4_controller_registry_("infantry4");

bool Infantry4Controller::Initialize() {
    // Common initialization.
    if (!Controller::Initialize("infantry4"))
        return false;

    // Initialize painter.
    controller_infantry4_debug_.Initialize(CmdlineArgParser::Instance().DebugShowImage());

    // Initialize Rune module.
    Frame init_frame;
    image_provider_->GetFrame(init_frame);
    if (rune_detector_.Initialize("../config/infantry4/rune-detector-param.yaml"))
    ...
    rune_predictor_.Initialize("../config/infantry4/rune-predictor-param.yaml");

    LOG(INFO) << "Infantry controller is ready.";
    return true;
}

void Infantry4Controller::Run() {
    sleep(2);
    ArmorPredictor armor_predictor{Entity::kBlue, "infantry4"};
    while (!exit_signal_) {
        if (!GetImage<true>())
            continue;
...
//            power_rune_ = rune_detector_.Run(receive_packet_.color, frame_);
            ...
            controller_infantry4_debug_.DrawAutoAimRune(frame_.image, &rune_predictor_, "detector rune network", 1);
        }
