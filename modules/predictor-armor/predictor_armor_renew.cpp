...
//            auto target(SameArmorByDistance(*begin,preprocessed_robots,kDistanceThreshold));
            ...
            if (matched_armor.second != nullptr) {
                begin->Predict(*matched_armor.first, delta_t, bullet_speed, battlefield.YawPitchRoll(),
                               ArmorPredictorDebug::Instance().ShootDelay());
...
std::pair<std::vector<Armor>::iterator, std::vector<Armor> *>
PredictorArmorRenew::SameArmorByDistance(const Armor &target,
...
std::pair<std::vector<Armor>::iterator, std::vector<Armor> *>
PredictorArmorRenew::SameArmorByPicDis(const Armor &target,
                                       std::unordered_map<Robot::RobotTypes, std::vector<Armor>> &robots,
                                       double threshold) {
    std::vector<Armor> *armors_point(nullptr);
    std::vector<Armor>::iterator target_iter;
