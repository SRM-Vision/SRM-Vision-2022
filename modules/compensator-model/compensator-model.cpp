...
#include "compensator-model.h"

bool CompensatorModel::Initialize(const std::string &type) {
    ...
    auto air_resistance_model = trajectory_solver::AirResistanceModel();
...
Eigen::Vector3d CompensatorModel::GetOffset(float pitch, double bullet_speed, const Armor &armor) {
    constexpr double min_theta = -CV_PI / 2, max_theta = CV_PI / 2, max_error = 0.01;
    constexpr unsigned int max_iter = 16;

    auto &tvc = armor.TranslationVectorCam();

    double target_pitch = atan(-tvc.y() / tvc.z()) + pitch;
...
//    delta_h = - armor.TranslationVectorCam().y() + target_d * sin(pitch);
